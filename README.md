# devoxxFR-2017
Restitution Devoxx 2017

## Travail
- [10 méthodes pour rendre heureux les développeurs, la 7e va vous étonner !](dev_heureux.md)

## Docker 
- [Docker 2017: Nouveautés et perspectives](docker_01_2017.md)
- [Kit d'orchestration avec docker swarm mode](docker_02_swarm.md)
- [Orchestration de conteneurs : le choix des armes !](docker_03_orchestration.md)

## Java
- [Java 9 modulo les modules](java_01_java_9.md)
- [L'API Collector dans tous ses états](java_02_API_collectors.md)
- [Jva 8 c'est bien, Javaslang c'est mieux ;-)](java_03_javaslang.md)
- [Pattern Matching en Java (10 ?) ](java_04_pattern_matching.md)
- [Spring Framework 5.0](spring_5.md)
- [Byte Buddy : la génération de bytecode facile !](bytebuddy.md)

## Variés
- [HTTP/2 du point de vue développeur](http2.md)
- [GraphQL vs Falcor](graphql_vs_falcor.md)
- [CQRS/EventSourcing par la pratique](CQRS.md)
